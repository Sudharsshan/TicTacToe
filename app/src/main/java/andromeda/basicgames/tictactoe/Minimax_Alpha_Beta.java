...
public class Minimax_Alpha_Beta extends MainActivity{

    public static int findBestMove(char[] board) {
        Log.d("Minimax algorithm report", "Process started");
        ...
        Log.d("Computer move is", "="+bestMove);
        return bestMove;
    }
    ...
    private static int evaluate(char[] board) {
        // Check for a win
        if (isWinner(board, computerChoice)) {
            return 10; // High positive score for the maximizing player (computer)
        } else if (isWinner(board, userChoice)) {
    ...
    protected static boolean isWinner(char[] board, char player) {
    ...
    private static int evaluateLine(char[] board, int pos1, int pos2, int pos3) {
        ...
        if (board[pos1] == computerChoice) {
            score = 1;
        } else if (board[pos1] == userChoice) {
            score = -1;
        }

        if (board[pos2] == computerChoice) {
            if (score == 1) {
